...
function EditPriceModal({ item, onClose }) {
  ...
  const margin = sellingPrice && costPrice ? Math.round(((sellingPrice-costPrice)/sellingPrice)*100) : null
  ...
  return (
    <Modal isOpen onClose={onClose} title={`Edit Price — ${item.name}`} size="sm"
      ...
      <div className="space-y-3">
        <div className="card-blue text-sm">
          Current: <span className="text-primary font-bold">{fmt.currency(item.sellingPrice)}</span>
        </div>
        <div>
        ...
        <div>
          ...
          <textarea value={reason} onChange={(e)=>setReason(e.target.value)} className="input-field resize-none" rows={2} placeholder="Reason…" />
        </div>
      </div>
    </Modal>
  )
...
export default function RatesTab() {
  ...
  const { data: inventory,  loading: l1, error  } = useFirestoreSubscription((cb,e)=>inventoryService.subscribe(cb,e))
  const { data: priceAudit, loading: l2 }          = useFirestoreSubscription((cb,e)=>auditService.subscribePriceAudit(cb,e))

  if (l1||l2) return <PageLoader />

  const categories = ['all',...new Set(inventory.map(i=>i.category))]
  const filtered = inventory.filter(i=>(!search||i.name.toLowerCase().includes(search.toLowerCase()))&&(catFilter==='all'||i.category===catFilter))
    ...
    <div className="p-4 max-w-7xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {[{id:'catalog',l:'📋 Price List'},{id:'audit',l:'🕐 Audit Log'}].map(({id,l})=>(
      ...
      {view==='catalog' ? (
              ...
              return (
                <div key={item.id} className="card hover:shadow-card-hover transition-shadow flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {item.status!=='active' && <span className="badge badge-gray text-xs capitalize">{item.status}</span>}
                  </div>
                  <div className="text-center hidden sm:block w-20"><p className="text-ink-4 text-xs">Cost</p><p className="text-ink-3 text-sm">{item.costPrice>0?fmt.currency(item.costPrice):'—'}</p></div>
                  <div className="text-center w-20"><p className="text-ink-4 text-xs">MRP</p><p className="font-bold text-primary text-sm">{fmt.currency(item.sellingPrice)}</p></div>
                  <div className="text-center w-16"><p className="text-ink-4 text-xs">Margin</p><p className={`font-semibold text-sm ${margin===null?'text-ink-4':margin<0?'text-danger':margin<20?'text-warning':'text-success'}`}>{margin!==null?`${margin}%`:'—'}</p></div>
        ...
        <div className="space-y-2">
          {priceAudit.length===0
            ? <div className="card text-center py-10 border-dashed"><p className="text-ink-3">No price changes yet</p></div>
            : priceAudit.map(entry=>(
              <div key={entry.id} className="card">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <p className="text-ink font-semibold text-sm">{entry.itemName}</p>
                ...
                <div className="flex items-center gap-3 mt-1.5 text-sm">
                  ...
                  <span className="font-bold text-primary">{fmt.currency(entry.newPrice)}</span>
