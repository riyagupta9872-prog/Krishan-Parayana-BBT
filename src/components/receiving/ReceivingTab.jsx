...
function RecordReceivingModal({ isOpen, onClose, inventory, isSuperAdmin }) {
  ...
  useEffect(() => {
    if (selectedItem && isSuperAdmin) setCostPerUnit(selectedItem.costPrice || '')
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input value={invoiceRef} onChange={(e) => setInvoiceRef(e.target.value)} className="input-field" />
          </div>
        </div>
        <div>
...
export default function ReceivingTab() {
  ...
  useEffect(() => {
    setLoading(true)
    const u1 = receivingService.subscribe((d) => { setEntries(d); setLoading(false) })
    const u2 = inventoryService.subscribe(setInventory)
    return () => { u1(); u2() }
  }, [])
    ...
    <div className="p-4 max-w-7xl mx-auto space-y-4">
      ...
      {entries.length === 0 ? (
        <div className="card text-center py-14 border-dashed">
          ...
          <p className="text-ink-3 font-body text-sm">No receiving entries yet.</p>
            ...
            <div key={e.id} className="card hover:shadow-card-hover transition-shadow">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-primary-lt border border-border-blue flex items-center justify-center text-xl shrink-0">
                    📦
                  </div>
                  <div>
                    <p className="text-ink font-body font-semibold text-sm">{e.itemName}</p>
                ...
                <div className="text-right shrink-0">
                  <p className="font-body font-bold text-success text-base">+{e.qtyReceived} units</p>
                  {isSuperAdmin && e.costPerUnit > 0 && (
                    <>
                      <p className="text-ink-3 text-xs">{fmt.currency(e.costPerUnit)}/unit</p>
                      <p className="text-primary text-xs font-medium">Total: {fmt.currency(e.totalCost)}</p>
                    </>
                  )}
                </div>
              </div>
